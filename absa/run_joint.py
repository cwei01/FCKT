    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if random_number < args.random_train:
            loss = model('train', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                         ...
                         polarity_labels=labels, label_masks=label_masks,weight_start=args.weight_start,weight_end=args.weight_end,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % save_checkpoints_steps == 0 and global_step > start_save_steps and count != 0:  # eval & save model
                ...
                print("P_all: {:.4f}, R_all: {:.4f}, F1_all: {:.4f}".format(metrics['p_all'], metrics['r_all'],
                                                                            metrics['f1_all']))
                print('>' * 30)
...
def main():
    ...
    ## ----------------------------------init-------------------------------
    parser.add_argument("--num_train_epochs", default=None,type=float)
    ...
    parser.add_argument("--logit_threshold", default=None, type=float)  # res_9.6
    parser.add_argument("--random_train", default=None, type=float)
    parser.add_argument("--weight_span", default=None, type=float)
